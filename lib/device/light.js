...
export default class {
  constructor(platform, accessory) {
    ...
    const kelvinOverrides = { H6008: { min: 2700, max: 6500 } }
    const kelvinOverride = kelvinOverrides[accessory.context.gvModel]
    this.minKelvin = kelvinOverride?.min ?? accessory.context?.supportedCmdsOpts?.colorTem?.range?.min ?? 2000
    this.maxKelvin = kelvinOverride?.max ?? accessory.context?.supportedCmdsOpts?.colorTem?.range?.max ?? 9000
